...
API_PREDICT_URL = f"{API_HOST}/predict"
API_CLIENTS_URL = f"{API_HOST}/clients"
...
def fetch_client_ids():
    ...
    try:
        ...
        st.warning("Vérifiez que le serveur Uvicorn est bien démarré et écoute sur `http://localhost:8000`.")
        return []
...
def main():
    ...
    st.markdown("""
        ...
        ⚠️ **Rappel:** L'API (FastAPI) doit être démarrée séparément (via `uvicorn api:app --host 0.0.0.0 --port 8000`).
    """)
    ...
    if st.button("Obtenir la Prédiction"):
        ...
        with st.spinner(f"Envoi de la requête pour l'ID {client_id} à l'API..."):
            ...
            try:
                ...
                st.warning("Vérifiez que le serveur Uvicorn est bien démarré et écoute sur l'adresse correcte.")
